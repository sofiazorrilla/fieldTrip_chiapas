...
ch = filter(dat, stateProvince == "Chiapas") %>% mutate(interest = ifelse(specificEpithet %in% c("insignis","skinneri","corrugata"),specificEpithet,"no"))
oax = filter(dat, stateProvince == "Oaxaca")

# Archivos para meter en gmaps
write.csv(dat,"data/sp_coord2.csv", row.names = F)
write.csv(ch[,-9],"data/ch_coord2.csv", row.names = F)
